define(['require','question-module'],function(require,module){
	...
	var createResponse=function(identifier){
		return {"identifier":identifier,"cardinality":"single","base_type":"identifier","corrects":[choiceIdentifiers[0]]};
	};
	...
	module.directive("lcChoiceItem", ['$identifier','$console','$i18n',function($identifier,$console,$i18n){
		return {
			restrict:'EA',
			replace:true,
			scope:{
			...
			templateUrl:'qti/$directives/lcChoiceItem.html',
				...
				$scope.removeChoice=function(c){
					$console.confirm($i18n('common.hint.delete'),function(){
						...
						for(var i=0,len=a.item.choices.length;i<len;i++){
							if(deleted!=-1){
								if($scope.answer.value==a.item.choices[i].identifier){
									$scope.answer.value=choiceIdentifiers[i-1];
								}
								a.item.choices[i].identifier=choiceIdentifiers[i-1];
							}else if(a.item.choices[i]==c){
								if($scope.answer.value==c.identifier){
			...
			link : function(scope, iElement, iAttrs) {
				...
				if(scope.assessment.response.corrects){
					scope.answer.value=scope.assessment.response.corrects[0];
				}
				scope.$watch('answer.value',function(newV){
					scope.assessment.response.corrects=[newV];
				});
				...
				scope.assessment.beforeCommit=function(a,errors){
					...
					for(var i=0;i<choices.length;i++){
						...
						console.log(trimText(choices[i].text));
