            ...
            if (value.answers && value.answers[0].length > 0) {
                for (var index = 0; index < value.answers[0].length; index++) {
                    ...
                    if (object && object.userIds) {
                        ...
                        if (value.questionType && (value.questionType == "fraction" || value.questionType == "textselect" || value.questionType == "nd_intervalproblem" || value.questionType == "nd_mathaxis")) {
                            if (data && data.isCorrect) {
                                ...
                                if (data && data.correct_response && data.user_response) {
                                    var is_same = false;
                                    if (typeof data.correct_response == 'string') {
                                        is_same = data.correct_response == data.user_response;
                                    }
                                    else {
                                        is_same = (data.correct_response.length == data.user_response.length)
