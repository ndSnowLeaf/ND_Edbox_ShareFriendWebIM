(function() {
	 ...
	 * @param {Object} shape
	 */
	ShapeBuilder.prototype.line = function(x1,y1,x2,y2, strokeColor, strokeWidth){
		//1、判断当前图形是否为直线
		...
		//2、重绘直线
		this.refresh(x1,y1,x2,y2, strokeColor, strokeWidth);
		return this;
	};
	 ...
	 * @param {Object} width
	 */
	ShapeBuilder.prototype.stroke = function(color, width){
		this.mRender.stroke(color, width);
		return this;
	};
	 ...
	 * 修改默认属性
	 */
	 ShapeBuilder.prototype.setAttr = function(attr){
	 	this.mRender.setAttr(attr);
	 	return this;
	 };
	...
	var Render = (function(){
		 ...
		 * @param {Object} color
		 */
		BasicRender.prototype.stroke = function(color, width){
			if(this.mInstance && this.onStroke){
				this.onStroke(color, width);
			}
		};
		 ...
		 * @param color
		 */
		BasicRender.prototype.onStroke = function(color, width){
			if(this.mInstance){
				var option = {
					stroke: color
				};
		...
		BasicRender.prototype.setAttr = function(attr){
			for(var key in attr){
				if(this.defaultAttr.hasOwnProperty(key)){
		...
		LazyRender.prototype.supportShape = {
			...
			equal:{
				name:'equal',
				str:'<svg width="150" height="150" xmlns="http://www.w3.org/2000/svg">' +
				'<g><path d="m1.12676,25.51123l147.71831,0l0,43l-147.71831,0l0,-43z"/><path d="m1.12676,81.9063l147.71831,0l0,43l-147.71831,0l0,-43z"/></g>' +
				'</svg>'
			},
			morethan:{
				name:'morethan',
			...
			lessequal:{
				name:'lessequal',
				str:'<svg width="150" height="150" xmlns="http://www.w3.org/2000/svg">' +
				'<g><path d="m148 0 l-148 60 l148 60 l0 -25 l-90 -35 l90 -35z"/><path d="m148 130 l-146 -60 l0 20 l146 60z"/></g>' +
				'</svg>'
			},
			moreequal:{
				name:'moreequal',
				str:'<svg width="150" height="150" xmlns="http://www.w3.org/2000/svg">' +
				'<g><path d="m2 0 l148 60 l-148 60 l0 -25 l90 -35 l-90 -35z"/><path d="m2 130 l146 -60 l0 20 l-146 60z"/></g>' +
				'</svg>'
			},
			parallelogram:{
				name:'parallelogram',
		...
		Line.prototype.onDraw = function(){
			...
			}else if(this.defaultAttr["stroke-dasharray"]){
				delete this.defaultAttr["stroke-dasharray"];
			}
			...
			if(this.mOption.isRendered === 0) {//初次渲染
				...
				// 线段描边
				if(this.line.strokeColor && this.line.strokeWidth){
					if(!this.line.terminal.stroke){
						this.line.terminal.stroke = this.snap.line(x1, y1, x2, y2).attr({stroke: this.line.strokeColor, strokeWidth: this.line.strokeWidth * 2});
						this.line.terminal.stroke.insertBefore(this.mInstance);
					}else{
						this.line.terminal.stroke.attr(option);
					}
					if(terminalAttr){
						terminalAttr.fill = this.line.strokeColor;
					}
				}
				if(x1 != x2 || y1 != y2){//只要长度不为0,就要画端点
					this.line.terminal.start.x = x1,this.line.terminal.start.y = y1;
		...
		Line.prototype.onRefresh = function(x1,y1,x2,y2, strokeColor, strokeWidth){
			console.log("onRefresh:",arguments);
			var w = Math.abs(x1 - x2),h = Math.abs(y1 - y2);
			var width = w<this.defaultAttr.strokeWidth*4?this.defaultAttr.strokeWidth*4:w;
			var height = h<this.defaultAttr.strokeWidth*4?this.defaultAttr.strokeWidth*4:h;
			this.snap.attr({
				width:width,
				height:height
			});
			...
			if(x2 > x1){
				this.line.x1 = this.defaultAttr.strokeWidth;
				this.line.x2 = w<this.defaultAttr.strokeWidth?this.defaultAttr.strokeWidth:w-this.defaultAttr.strokeWidth;
			}else if(x1 == x2){
				this.line.x1 = this.defaultAttr.strokeWidth*2;
				this.line.x2 = this.defaultAttr.strokeWidth*2;
			}else{
				this.line.x1 = w<this.defaultAttr.strokeWidth?this.defaultAttr.strokeWidth:w-this.defaultAttr.strokeWidth;
				this.line.x2 = this.defaultAttr.strokeWidth;
			}

			if(y2 > y1){
				this.line.y1 = this.defaultAttr.strokeWidth;
				this.line.y2 = h<this.defaultAttr.strokeWidth?this.defaultAttr.strokeWidth:height-this.defaultAttr.strokeWidth;
			}else if(y1 == y2){
				this.line.y1 = this.defaultAttr.strokeWidth*2;
				this.line.y2 = this.defaultAttr.strokeWidth*2;
			}else{
				this.line.y1 = h<this.defaultAttr.strokeWidth?this.defaultAttr.strokeWidth:height-this.defaultAttr.strokeWidth;
				this.line.y2 = this.defaultAttr.strokeWidth;
			}

			if(strokeColor && strokeWidth){
				this.line.strokeColor = strokeColor;
		...
		Halfcircle.prototype.onDraw = function(){
			...
			path = "M " + width/2 + " " + this.defaultAttr.strokeWidth/2
				+ " A " + (width - this.defaultAttr.strokeWidth)/2 + " " + (height - this.defaultAttr.strokeWidth)/2
				+ " 0 0 0 " + width/2 + " " + (height - this.defaultAttr.strokeWidth/2) + " Z";
			if(this.mOption.isRendered === 0) {//初次渲染
				this.defaultAttr.fill = "#fff";
				this.mInstance = this.snap.path(path).attr(this.defaultAttr);
			}else{//二次渲染
			...
			var m = new Snap.Matrix().rotate(-45,width/2,height/2);
